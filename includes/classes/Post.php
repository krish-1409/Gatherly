...
class Post {
	...
	public function submitPost($body, $user_to, $imageName) {
		$body = strip_tags($body);                              //removes html tags in the posts
		$body = mysqli_real_escape_string($this->con, $body);
		$body = str_replace('\r\n', "\n", $body);            // to remove multiple mpt lines
		$body = nl2br($body);
		$check_empty = preg_replace('/\s+/', '', $body);                   //To Deltes all spaces 
...

		{
			...
			$body = implode(" ", $body_array);  //to convert a list or array into string
		...
		if(mysqli_num_rows($data_query) > 0) 
		{
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				if($row['user_to'] == "none") {
					...
					$user_obj = new User($this->con, $row['user_to']);
